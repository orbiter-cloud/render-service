...
const levelToSpace = level => Array(level).fill(null).map(() => '  ').join('');

const fsMkDir = path => new Promise((resolve, reject) => {
  fs.mkdir(path, {
    recursive: true
  }, err => {
    if (err) {
...
const fsWrite = (path, data, level = 0) => new Promise((resolve, reject) => {
...
export const renderCommand = async (_c, args, commandRun) => {
  ...
  const renderFile = async file => {
    ...
    const res = await renderClient.render('default', 'en', 'main', renderClient.tplRef('demo', 'pages/default'), renderClient.optimize('html'), dataFileContent, {}, {}, commandRun.runId);
  ...
  const renderStyle = async styleId => {
    const res = await renderClient.style('default', styleId, 'demo', {
      nanoCss: true,
      cssAutoPrefix: true
    }, {}, commandRun.runId);
